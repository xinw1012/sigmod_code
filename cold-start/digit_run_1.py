...
svm_mtl = lg.UserDefineModel(xs,ys,dy,ss) # using the default value 
svm_mtl.train_all_fs()
svm_mtl.train_ws()
svm_err = lg.seg_model_error_01(svm_mtl,test_xs,test_ys,test_ts)
print 'Initial Error SVM_mtl ', svm_err
svm_feature_errors = []
...
f.write('\n\t')    
for i in iters:
    print 'cold-start: # of points: ', i
    task = digits.create_mtl_datasets(Z,train_y,nTasks=1,taskSize=i,testSize=1000)
    test_xs,test_ys,test_ts = digits.generate_additional_data(task,oracle_mtl, svm_mtl,0)       
    oracle_mtl.train_ws()
    oracle_err = lg.seg_model_error_01(oracle_mtl, test_xs, test_ys, test_ts, num=20)
    oracle_mtl_errors.append(oracle_err)
    ...
    print 'Testing Error of svm model is ', svm_err
...
ax.plot(iters, svm_feature_errors, label = 'svm')
