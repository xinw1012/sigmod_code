...
class UserDefineModel:
    ...
    def predict(self, x, s, key, dic,i=10):
        if self.trained_fs and self.trained_ws:
            ...
            if key in dic:
                ...
                self.hit += 1
                feature = self.segments[s].avg_f
                dic[key] = self.compute_features(x,s,i).transpose()
                #print 'miss!'
...
def seg_model_error_01(model, xs, ys, ss, key, dic,  num = 10):
    wrong = 0.0
    for i in range(len(xs)):
        kk = str(key[i].tolist())
        y_pred = model.predict(xs[i], ss[i], kk, dic, num)
        y_true = ys[i]
        if y_pred != y_true:
            wrong += 1
    return wrong/len(xs)
